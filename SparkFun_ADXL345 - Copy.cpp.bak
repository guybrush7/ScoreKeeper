...
void ADXL345::disableAllInterrupts(void)
{
...
void ADXL345::readAccel(int *x, int *y, int *z) {
	...
	*/
	...
	xx = (((int16_t)_buff[1]) << 8) | _buff[0];   
	yy = (((int16_t)_buff[3]) << 8) | _buff[2];
	zz = (((int16_t)_buff[5]) << 8) | _buff[4];
...
void ADXL345::EnableTriggerMode(uint8_t int_pin, uint8_t preTrigSamples)
{
	byte r = 0xC0;
	...
	r |= (preTrigSamples & 0x1F);
	
	writeTo(ADXL345_FIFO_CTL, r);
}

void ADXL345::DisableTriggerMode(void)
{
	writeTo(ADXL345_FIFO_CTL, 0);
}

bool ADXL345::isFifoTriggered(uint8_t &fifoEntries)
{
	uint8_t r;
	
	readFrom(ADXL345_FIFO_STATUS, 1, &r);
...
void ADXL345::ResetTrigger(void)
{
	byte reg;
	// save value
	readFrom(ADXL345_FIFO_CTL, 1, &reg);
	
	// setting to bypass clears trigger
	writeTo(ADXL345_FIFO_CTL, 0);
	
	// restore trigger setting
	writeTo(ADXL345_FIFO_CTL, reg);
